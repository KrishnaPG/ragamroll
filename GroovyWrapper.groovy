...
/*
 * Adapted from: http://groovy.codehaus.org/WrappingGroovyScript
*/
...
cli.m( longOpt: 'mainclass', argName: 'mainclass', required: true, args: 1, 'fully qualified main class, eg. RagaImitator' )
cli.c( longOpt: 'groovyc', required: false, 'Run groovyc' )

//--------------------------------------------------------------------------
def dependencies = [
  'ragamroll' : [ 'NotationParser' ],
  'NotationParser' : [],
  'RagaAnalyzer' : ['NotationParser'],
  'RagaImitator' : ['RagaAnalyzer', 'NotationParser']
]
...
def libFiles = dependencies[mainClass].collect { f ->
  new File( f + '.groovy' )
}

def libFile = new File( 'NotationParser.groovy' )
...
if (opt.c) {
  libFiles.each { lf1 ->
    ant.echo( "Compiling ${lf1}" )
    org.codehaus.groovy.tools.FileSystemCompiler.main( [ lf1 ] as String[] )
  }
  //ant.echo( "Compiling ${libFile}" )
  //org.codehaus.groovy.tools.FileSystemCompiler.main( [ libFile ] as String[] )
...
ant.jar( destfile: destFile, compress: true, index: true ) {
  ...
  fileset( dir: '.', includes: 'raga_base.txt' )
  dependencies[mainClass].each { lf2 ->
    fileset( dir: '.', includes: lf2 + '*.class' )
  }

  zipgroupfileset( dir: GROOVY_HOME, includes: 'embeddable/groovy-all-*.jar' )
  zipgroupfileset( dir: GROOVY_HOME, includes: 'lib/commons*.jar' )
  zipgroupfileset( dir: './ext-libs', includes: 'jfugue-4.1.0-2011-02-03.jar' )
  // add more jars here
